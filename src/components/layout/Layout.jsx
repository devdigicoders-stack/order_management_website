...
export const Layout = () => {
  ...
  return (
    <div className="flex h-[100dvh] w-screen bg-gray-50 overflow-hidden">
      {/* Desktop Sidebar */}
      <div className="w-64 bg-gray-900 text-white flex-shrink-0 hidden md:flex flex-col">
        <div className="h-16 flex items-center px-6 border-b border-gray-700">
          <PackageSearch className="w-6 h-6 text-blue-400 mr-3" />
          <span className="text-xl font-semibold">OMS System</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-5 px-3 space-y-1">
          {filteredNavItems.map((item) => (
            ...
            >
              <item.icon className={clsx("w-5 h-5 mr-3", { "text-blue-300": true })} />
              {item.name}
            </NavLink>
          ))}
          {user?.role === 'staff' && (
            ...
            >
              <div className="flex items-center">
                <Bell className={clsx("w-5 h-5 mr-3", { "text-blue-300": true })} />
                Notifications
              </div>
              {unreadCount > 0 && (
        ...
        <div className="p-4 border-t border-gray-700 bg-gray-800/50">
          <div className="flex items-center justify-between">
            <Link to="/profile" className="flex items-center space-x-3 hover:bg-gray-700 p-2 -ml-2 rounded-md transition-colors flex-1 overflow-hidden">
              <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center font-semibold text-sm shadow-sm flex-shrink-0">
            ...
            <button 
              ...
              className="p-2 rounded-md hover:bg-gray-700 text-gray-400 hover:text-red-400 transition-colors ml-1 flex-shrink-0"
              title="Logout"
            >
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-white border-b border-gray-200 z-40 flex items-center justify-between px-4 shadow-sm">
        <div className="flex items-center">
          <PackageSearch className="w-5 h-5 text-blue-600 mr-2" />
          <span className="font-bold text-gray-900">OMS</span>
        </div>
        <div className="flex items-center space-x-3">
          ...
          <Link to="/profile" className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center font-semibold text-blue-700 text-xs shadow-sm">
            {user?.name?.charAt(0) || 'U'}
          </Link>
        </div>
      </div>
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col h-full overflow-hidden relative mt-14 md:mt-0">
        ...
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative flex-1 flex flex-col max-w-xs w-full bg-white shadow-xl animate-in slide-in-from-left duration-200">
            <div className="h-16 flex items-center justify-between px-6 border-b border-gray-100">
              <div className="flex items-center">
                <PackageSearch className="w-6 h-6 text-blue-600 mr-3" />
                <span className="text-xl font-bold text-gray-900">OMS Menu</span>
              </div>
              <button 
                onClick={() => setIsMobileMenuOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto py-5 px-4 space-y-1">
              {filteredNavItems.map((item) => (
                ...
                >
                  {({ isActive }) => (
                    <>
                      <item.icon className={clsx("w-5 h-5 mr-4", isActive ? "text-blue-600" : "text-gray-400")} />
                      {item.name}
                    </>
                  )}
                </NavLink>
