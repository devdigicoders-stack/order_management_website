...
const EditableOrderCell = ({ getValue, row, column, table }) => {
  ...
  return (
    <input
      ...
      onBlur={onBlur}
      className={`w-16 border rounded text-center text-sm py-1 focus:ring-blue-500 focus:border-blue-500 shadow-sm ${value > 0 ? 'border-blue-300 bg-blue-50' : 'border-gray-200'}`}
      min="0"
    />
  );
};
  ...
  return (
    <div className="space-y-6 h-full md:h-full flex flex-col mb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Orders Matrix Dashboard</h1>
          <p className="text-gray-500 text-sm mt-1">
            {user?.role === 'staff' ? 'Enter your orders directly into the grid.' : 'Live Excel-style view of all active orders.'}
          </p>
        </div>
        <div className="flex flex-col md:flex-row flex-wrap gap-3 w-full md:w-auto">
          <div className="relative w-full md:w-auto">
            ...
            <input 
              ...
              className="w-full md:w-48 pl-9 pr-4 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500"
            />
          </div>
          <div className="flex gap-2 w-full md:w-auto">
            <select 
              ...
              className="flex-1 md:flex-none border-gray-300 rounded-md shadow-sm text-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500 border bg-white"
            >
              {companies.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <button className="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-50 flex items-center shadow-sm shrink-0">
          ...
          {user?.role === 'staff' ? (
            <button 
              ...
              className="w-full md:w-auto justify-center bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700 shadow-sm flex items-center mt-1 md:mt-0"
            >
              Submit Orders
            </button>
          ) : (
            <button className="w-full md:w-auto justify-center bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700 shadow-sm flex items-center mt-1 md:mt-0">
