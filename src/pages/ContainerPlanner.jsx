...
export const ContainerPlanner = () => {
 ...
 <div>
 <h1 className="text-xl font-semibold text-white ">Container Planner</h1>
 <p className="text-indigo-100 text-sm mt-1.5 opacity-90">Manually assign products to containers.</p>
 </div>
 <button 
 onClick={() => setShowModal(true)}
 className="bg-white text-indigo-700 px-5 py-2 rounded-lg text-sm font-semibold hover:bg-indigo-50 shadow-md flex items-center justify-center transition-transform hover:scale-105 shrink-0"
 >
 ...
 {/* Unassigned Items */}
 <div className="lg:col-span-4 bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-200 flex flex-col h-full">
 ...
 <span className="bg-indigo-100 text-indigo-700 text-xs px-2.5 py-1 rounded-full font-semibold shadow-sm border border-indigo-200">
 {unassignedItems.length}
 </span>
 </div>
 <div className="flex-1 overflow-y-auto p-4 space-y-3">
 {unassignedItems.map(item => (
 <div key={item.id} className={`border rounded-lg p-3 transition-colors cursor-pointer group ${selectedItems[item.id] ? 'border-indigo-500 bg-indigo-50/30' : 'border-gray-200 hover:border-indigo-300'}`}>
 ...
 {item.remainingCuft.toFixed(2)} <span className="text-[10px] text-indigo-400 font-medium ml-1">cuft</span>
 </div>
 </div>
 </div>
 </div>
 ...
 <label className="text-[10px] font-semibold tracking-wider text-gray-500 mb-1.5 uppercase">Move To</label>
 <div className="relative w-32">
 <select 
 className="appearance-none w-full bg-white border border-gray-300 text-gray-900 text-sm font-semibold rounded-lg pl-3 pr-8 py-2 outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 cursor-pointer text-center shadow-sm hover:shadow-md transition-shadow duration-300 transition-colors hover:border-gray-400"
 ...
 ? 'bg-indigo-600 text-white shadow-md hover:bg-indigo-700 cursor-pointer transform hover:scale-105' 
 ...
 return (
 <div key={container.id} className={`bg-white rounded-xl shadow-sm border transition-colors ${isTarget ? 'border-indigo-400 ring-1 ring-indigo-400' : 'border-gray-200'}`}>
 <div className={`p-5 border-b border-gray-100 rounded-t-xl ${isTarget ? 'bg-indigo-50/50' : 'bg-gradient-to-r from-gray-50 to-white'}`}>
 <div className="flex justify-between items-end mb-4">
 <div>
 <h2 className="font-semibold text-gray-900 text-lg flex items-center ">
 <Container className={`w-5 h-5 mr-2 ${isTarget ? 'text-indigo-600' : 'text-indigo-500'}`} />
 {container.name}
 {isTarget && <span className="ml-3 bg-indigo-100 text-indigo-700 text-[10px] px-2.5 py-1 rounded-full font-semibold uppercase tracking-wider border border-indigo-200 shadow-sm">Target</span>}
 </h2>
 <p className="text-sm font-medium text-gray-500 mt-1">Capacity: {container.capacity} cuft</p>
 </div>
 <div className="text-right">
 <p className="text-[10px] text-gray-400 uppercase font-semibold tracking-widest mb-1">Remaining</p>
 <p className={`text-xl font-semibold ${remainingCuft < 0 ? 'text-red-600' : 'text-green-600'}`}>
 {remainingCuft.toFixed(2)} <span className="text-sm font-semibold opacity-70">cuft</span>
 </p>
 </div>
 </div>
 ...
 <label className="block text-[10px] font-semibold tracking-wider text-gray-500 mb-1 uppercase">Move {Object.keys(selectedItems).length} item(s) to</label>
 <div className="relative">
 <select 
 className="appearance-none w-full bg-white border border-gray-300 text-gray-900 text-sm font-semibold rounded-lg pl-3 pr-8 py-2 outline-none focus:ring-2 focus:ring-indigo-500"
 value={targetContainerId || ''}
 onChange={(e) => setTargetContainerId(e.target.value)}
 >
 {containers.map(c => (
 ...
 <button 
 onClick={handleAssign}
 className="mt-4 bg-indigo-600 text-white px-4 py-2 rounded-lg shadow-md hover:bg-indigo-700 font-medium flex items-center shrink-0"
 >
 ...
 <h3 className="text-base font-semibold text-gray-900">Add New Container</h3>
 ...
 className="w-full border-gray-300 rounded-lg shadow-sm p-2.5 border focus:border-indigo-500 focus:ring-indigo-500" 
 ...
 <button type="submit" className="px-4 py-2 bg-indigo-600 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white hover:bg-indigo-700 transition-colors">Add Container</button>
 </div>
 </form>
 </div>
 </div>
