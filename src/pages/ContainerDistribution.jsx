  ...
  return (
    <div className="flex flex-col items-center justify-center">
      ...
      <input
        ...
        onBlur={onBlur}
        className="w-16 border border-gray-300 rounded text-center text-sm py-1 focus:ring-blue-500 focus:border-blue-500 shadow-sm"
        min="0"
      />
    </div>
  );
  ...
  return (
    <div className="space-y-6 h-full md:h-full flex flex-col max-w-7xl mx-auto mb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Container Distribution</h1>
          <p className="text-gray-500 text-sm mt-1">Allocate received container items to staff members.</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center w-full md:w-auto">
          {isSaved && <span className="text-green-600 text-sm font-medium flex items-center"><CheckCircle className="w-4 h-4 mr-1"/> Saved</span>}
          <select 
            value={selectedContainer}
            onChange={(e) => setSelectedContainer(Number(e.target.value))}
            className="w-full sm:w-auto border-gray-300 rounded-md shadow-sm text-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500 border bg-white"
          >
            {containers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          <button 
            onClick={handleSave}
            className="w-full sm:w-auto justify-center bg-green-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-green-700 shadow-sm flex items-center transition-colors shrink-0"
          >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex-1 flex flex-col min-h-0">
        <div className="p-4 border-b border-gray-200 bg-gray-50 flex items-center flex-shrink-0">
